...
PREPROD_ALB_URL = "http://pricing-exch.alb.prebb.net/pricing-exch/v1/autoprices/current_price"
PREPROD_URL = "http://52.77.211.221:8080/pricing-exch/v1/autoprices/current_price"
PROD_URL = "http://pricing-exch.alb.jinka.in/pricing-exch/v1/autoprices/current_price"
LOCAL_FILE="data/order-reports_28_09_2017_22_47.csv.zip"
...
PRODUCTS_MAP ={
 ...
 'Powder (Bags) - Big Factory': {'ID': 142,
  'TYPE': 'Powder (Bags) - Big Factory'},
 'Powder (Bags) - Medium Factory': {'ID': 143,
  'TYPE': 'Powder (Bags) - Medium Factory'},
 'Powder (Bags) - Small Factory': {'ID': 144,
  'TYPE': 'Powder (Bags) - Small Factory'},
 'Powder (Loose)': {'ID': 145, 'TYPE': 'Powder (Loose)'},
 'Powder Loose (Non - Sample)': {'ID': 168,
  'TYPE': 'Powder Loose (Non - Sample)'},
...
TT_INFO_MAP={
    ...
    "40ft MAC"          :{"id":14, "body_type":"CONTAINER",  "capacity":[],      "length":40.00 },
    ...
    "32ft HQ"           :{"id":100,"body_type":"HQ",         "capacity":[],      "length":32.00},
    "Flat Bed 20ft"     :{"id":101,"body_type":"FLAT_BED",   "capacity":[],      "length":20.00},
    "Flat Bed 40ft"     :{"id":102,"body_type":"FLAT_BED",   "capacity":[],      "length":40.00},
    "Tanker"            :{"id":103,"body_type":"TANKER",     "capacity":[],      "length":-1},
    "Warm Tanker"       :{"id":104,"body_type":"WARM_TANKER","capacity":[],      "length":-1},
    "Bulker"            :{"id":105,"body_type":"BULKER",     "capacity":[],      "length":-1},
    "Dumper"            :{"id":106,"body_type":"DUMPER",     "capacity":[],      "length":-1},
...
def putPredictValue(url, payload):
    ...
    assert r.status_code == 200, "Failed uploading data to pricing server"
...
def getSeriesWithResampleAndFNA(kpi, W=8):
    kpi_ = kpi[pd.isnull(kpi['ptr'])==False]
    # Lets Group By by ('From City', 'From Sublocation', 'To City', 'To Sublocation', 
    #'Used truck Type', 'Product Name')
    kpi_groupBy = kpi_.groupby(['From City', 'From Sublocation', 'To City', 'To Sublocation', 
                                'Used truck Type', 'Product Name'])
    kpi_groupBy = [x for x in list(kpi_groupBy) if len(x[1]) >= W]
    all_series = [(x[0], Series(x[1]['ptr'].values, index=x[1]['d'].values)) for x in kpi_groupBy]
...
def getSeriesWithoutResample(kpi, W=8):
    kpi_ = kpi[pd.isnull(kpi['ptr'])==False]
    # Lets Group By by ('From City', 'From Sublocation', 'To City', 'To Sublocation',
    #'Used truck Type', 'Product Name')
    kpi_groupBy = kpi_.groupby(['From City', 'From Sublocation', 'To City', 'To Sublocation',
...
def getFeaturesForAllSeriesWithSplit(all_series, W=7, train_frac = 0.5):
...
def makeJsonPayload(price, truck_name, product_type, from_sub, to_sub,  
                    price_unit='TON', price_type='SP', algo='ARIMA'):

    # make truck type object
    tt = {'body_type':[TT_INFO_MAP[truck_name]['body_type']]}

    # add capacity if it's there
    if TT_INFO_MAP[truck_name]['capacity']:
        tt['capacity'] = TT_INFO_MAP[truck_name]['capacity']

    # add Truck length if it's there  
    if TT_INFO_MAP[truck_name]['length'] != -1:
        tt['length'] = [TT_INFO_MAP[truck_name]['length']]
        

    from_sub_details = BASE_SUBLOCATIONS_MAP[from_sub]
    to_sub_details = BASE_SUBLOCATIONS_MAP[to_sub]
        
    d ={
        'from_sublocation_id' : from_sub_details["ID"],
        'from_location_id': from_sub_details["LOCATION_ID"],
        'to_sublocation_id' : to_sub_details["ID"],
        'to_location_id': to_sub_details["LOCATION_ID"],
        'price': price,
        'price_type': price_type,
        'price_unit': price_unit,
        'product_id': PRODUCTS_MAP[product_type]["ID"],
        'algo': algo,
        'comments': 'Setting Price using %s' %algo,
        'truck_type': tt
        }
...
def runIRFRForDF(df, W=7, T=1):
    rfr = RandomForestRegressor(n_estimators=100, 
                            ...
                            bootstrap=True, oob_score=False, n_jobs=-1, 
                            random_state=None, verbose=0)
    all_series = getSeriesWithoutResample(df)
    ((train_feat, train_labl), (test_feat, delta)) = getFeaturesForAllSeries(all_series)
    ...
    return {("%s:%s:%s:%s" %(t["From Sublocation"], t["To Sublocation"], t["Used truck Type"],
                               t["Product Name"])):(p, t) for t,p in zip(test_feat, pred_act)}
...
def makePredictionAndSave(kpi, from_sub, to_sub, truck_name, product_type, irfrVals):
    ...
    #print "FROM_SUBLOCATION, TO_SUBLOCATION, TRUCK_NAME, PRODUCT_TYPE, ARIMA_PRICE, RFR_PRICE"
    ...
    payload1 = makeJsonPayload(rfr_r, truck_name, product_type, 
                              from_sub, to_sub, price_unit, price_type, 'RFR')
    payload2 = makeJsonPayload(arima_r, truck_name, product_type, 
                              from_sub, to_sub, price_unit, price_type, 'ARIMA')
    payload3 = makeJsonPayload(irfr_r, truck_name, product_type, 
                              from_sub, to_sub, price_unit, price_type, 'IRFR')
           
           
    ### Now save this prediction into Server by HTTP Put
    #putPredictValue(PUT_URL, payload1)
    #putPredictValue(PUT_URL, payload2)
    #putPredictValue(PUT_URL, payload3)
...
def letsDoIt(date="09/28/2017"):
    ...
    for p in PARAM_TUPS:
        ...
        p["rfr"] = rfr
        p["irfr"] = irfr
        print "%s, %s, %s, %s, %s, %s, %s" %(p["from_sub"], p["to_sub"], p["truck_name"], p["product_type"], arima, rfr, irfr)
